...
import EditableShiftSection from './EditableShiftSection'
...
export default function DayOverview({ date, teams, allAvailability, memberRole, scriptUrl }: Props) {
  ...
  const isEditable = memberRole === 'project_lead'
          ...
          {[0, 1].map((si) => {
            ...
            return (
              <div key={si}>
                ...
                {isEditable ? (
                  <EditableShiftSection
                    date={date}
                    shiftIndex={si as 0 | 1}
                    teams={shiftTeams}
